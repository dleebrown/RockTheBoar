...
fc1_shape = [320*480*conv5_shape[3], 320*480*conv5_shape[3]]
...
# fully-connected layer block. takes in convlayer and flattens, then reshapes and returns same size after running
# fully-connected op
def fc_layer_block(inputs, weights, bias, keep_prob, drop=True, activation='relu'):
    shape = inputs.get_shape()
    nparams = shape[1:4].num_elements()
    inputs = tf.reshape(inputs, [-1, nparams])
    inputs = tf.matmul(inputs, weights)
    inputs += bias
    if activation == 'relu':
        activate = tf.nn.relu(inputs)
    else:
        activate = tf.nn.sigmoid(inputs)
    if drop:
        dropped = tf.nn.dropout(activate, keep_prob)
    else:
        dropped = activate
    outputs = tf.reshape(dropped, shape=[1, int(shape[1]), int(shape[2]), int(shape[3])])
    return outputs
...
with tf.name_scope(prefix+'WEIGHTS'):
    ...
    fcweight1, fcbias1 = initialize_fc_weights_bias(fc1_shape)
...
with tf.name_scope(prefix+'CV_LAYERS'):
    ...
    cv_block5 = conv_layer_block(cv_block4, cweight5, conv5_stride, cbias5, conv5_pstride, dropout, drop=True,
                                 maxpool=False)
    fc_block1 = fc_layer_block(cv_block5, fcweight1, fcbias1, dropout, drop=True)
    dc_block1 = deconv_layer_block(fc_block1, dcweight1, dcbias1, dc1_output_shape, dc1_stride)
